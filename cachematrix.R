...
makeCacheMatrix <- function(x = matrix()) { # sets x equal to an empty matrix
  ...
  } ## so far nothing has happened other than creating objects that live at the top of the heap
...
cacheSolve <- function(x, ...) {
  I<- x$getInverse()  # Retrieves the most recent value for the inverse
  if(!is.null(I)){     # # If the value of Inverse exists (!is.null) gei it from the cache
    message("getting cached data")
    return(I)        
  }
  # Otherwise retrieve matrix x and calculate the inverse with the R solve() function
